...
def health():
    ...
    commit_url = "https://github.com/mensasverige/swagapp/commit/" + commit_hash
...
def list_events():
    ...
    with open('static_events.json') as f:
        ...
        for event in static_events:
            event[
                'id'] = f"{event['name'].replace(' ', '-').lower()}-{event['start'].replace(' ', '-').lower()}"
            # add location {latitude, longitude} if not present
            if 'location' not in event or 'latitude' not in event[
                    'location'] or 'longitude' not in event['location']:
                event['location'] = {'latitude': 0, 'longitude': 0}
...
def users_showing_location():
    ...
    if False:
        return jsonify([{
            ...
            "name": "Carl Green",
            "avatar_url":
            "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQbG-0Pc_dX0swJiOnUTf58QaSAwwUTpBUi6Q&usqp=CAU",
            "location": {
    ...
    try:
        ...
        location_age_cutoff = datetime.datetime.now() - datetime.timedelta(
            hours=1)
        users = list(
            db.user.find({
                'show_location': True,
...
def update_user_location():
    ...
    try:
        ...
        if existing_user:
            db.user.update_one({"username": data["username"]},
                               {'$set': {
            ...
            return jsonify({'message':
                            'User location updated successfully'}), 200
        else:
            # This means that no document was found with the provided `_id`
            logging.warning(
            ...
            return jsonify(
                {'error':
                 'User not found or location already up to date'}), 404
    ...
    for filename in os.listdir(schema_dir):
        ...
        if filename.endswith('.json'):
            ...
            schema, collection = initialize_collection_from_schema(
                model_name, os.path.join(schema_dir, filename), db)

            logging.info(f"Initialized collection {model_name}")

            app.add_url_rule(f'/{model_name}',
                             f'create_{model_name}',
                             partial(create, model_name, schema, collection),
                             methods=['POST'])
            app.add_url_rule(f'/{model_name}',
                             f'read_{model_name}',
                             partial(read, model_name, collection),
                             methods=['GET'])
            app.add_url_rule(f'/{model_name}/<string:item_id>',
                             f'read_{model_name}_by_id',
                             partial(read, model_name, collection),
                             methods=['GET'])
            app.add_url_rule(f'/{model_name}/<string:item_id>',
                             f'update_{model_name}',
                             partial(update, model_name, schema, collection),
                             methods=['PUT'])
            app.add_url_rule(f'/{model_name}/<string:item_id>',
                             f'delete_{model_name}',
                             partial(delete, model_name, schema, collection),
                             methods=['DELETE'])
        ...
        with open('/review_user.txt', 'r') as f:
            ...
            result = db['user'].update_one({'username': review_user}, {
                '$set': {
                    'name': 'Reviewer',
                    'avatar_url':
                    'https://swag.k.mikael.blue/i-love-apple.png',
                }
            },
                                           upsert=True)
